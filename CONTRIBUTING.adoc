...
Every plugin configuration needs an associated manually written JSON schema.
Depending on the stage, we must add the following files

```
schema/source/<pluginKind>.json
schema/condition/<pluginKind>.json
schema/target/<pluginKind>.json
```

We must update the main schema `schema/config.json` referencing the previously mentioned JSON schema

Finally we must add an updatecli manifest example to `e2e/updatecli.d` to validate that our schema is working as expected
