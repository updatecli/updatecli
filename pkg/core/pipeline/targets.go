...
func (p *Pipeline) RunTarget(ctx context.Context, id string, sourceIds []string) (r string, changed bool, err error) {
	target := p.Targets[id]
	...
	err = target.Run(ctx, p.Sources[target.Config.SourceID].Output, &p.Options.Target)
	if err != nil {
