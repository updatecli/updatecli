...
func (m MatchingRules) Validate() error {
	for i, rule := range m {
		if rule.Path == "" && len(rule.Repositories) == 0 && len(rule.Artifacts) == 0 {
			return fmt.Errorf("rule %d has no valid fields (path, repositories, or artifacts must be specified)", i+1)
		}
	}
	return nil
}
