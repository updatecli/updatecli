	...
	for _, pomFile := range foundPomFiles {
		...
		for i, dependency := range dependencies {
			...
			if isContainsVariable {
				logrus.Printf("Skipping dependency as current version rely on variable %q", dependency.Version)
				continue
			}
			...
			manifest := config.Spec{
				...
				Targets: map[string]target.Config{
					artifactFullName: {
						...
						ResourceConfig: resource.ResourceConfig{
							Name: fmt.Sprintf("Bump Maven Artifact %q version", artifactFullName),
							Kind: "xml",
							Spec: xml.Spec{
								File: relativePomFile,
