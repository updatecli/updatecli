...
func (c *Chart) validateVersionInc() []error {
	...
	for _, inc := range versionIncrement {
		...
		case PATCHVERSION:
			...
			acceptedRules = append(acceptedRules, inc)
		default:
			errs = append(errs, fmt.Errorf("unrecognized increment rule %q, accepted values are a comma separated list of [major,minor,patch,child-dependent], or 'none' to disable version increment", inc))
		}
	}
