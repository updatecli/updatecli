...
// PEP 440 versions are normalized to semver-compatible form so that version filtering works.
// The original PEP 440 version is preserved in normalizedToOriginal for result mapping.
...
func (p *Pypi) getVersions() (string, []string, error) {
	...
	// Map back to original PEP 440 version for downstream consumers.
	found := p.originalVersion(p.foundVersion.GetVersion())
