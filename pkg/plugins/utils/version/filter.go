...
// NewFrilterFromValue tries to identify the closest versionFilter rule based on a value
func NewFilterFromValue(value string) *Filter {

	logrus.Debugf("Trying the identify the best versionFilter for %q", value)

	if value == "" {
		logrus.Debugln("=> empty value detected")
		return nil
	}
	...
	for _, pattern := range patterns {
		...
		if re.MatchString(value) {
			switch pattern.rule {
			// If For strict semver, we want to return semver versionfilter
			case `^\d*(\.\d*){2}$`:
				logrus.Debugln("=> strict semantic versioning without metadata or prerelease information detected")
				return &Filter{
					Kind:   SEMVERVERSIONKIND,
					Strict: true,
				}
			case `^v\d*(\.\d*){2}$`:
				logrus.Debugln("=> looks like semantic versioning with a v prefix")
				return &Filter{
					Kind: SEMVERVERSIONKIND,
				}
			default:
				...
				logrus.Debugf("=> closest regex %q identify for value %q", newRule, value)
				return &Filter{
	...
	logrus.Debugln("=> No matching rule identified, feel free to suggest one https://github.com/updatecli/updatecli/issues/new/choose")
	logrus.Debugf("we couldn't identify pattern for value %q\n", value)
